...
for g in g_values:
    ...
    for trial in range(N_TRIALS):
        ...
        # (fixes the g=0.42 false-negative risk from the original 0.18 floor).
                # Per-run adaptive peak threshold
        run_peak_floor = 0.5 * np.max(C_arr)
        
        # CRITICAL TWEAK: Enforce a minimum distance between peaks to prevent 
        # thermal noise from splitting a single oscillation peak into two.
        # True period is ~3.14, so 2.0 is a safe lower bound.
        min_peak_distance = int(2.0 / dt_sample)
        
        peaks, _ = find_peaks(C_arr, height=run_peak_floor, distance=min_peak_distance)
